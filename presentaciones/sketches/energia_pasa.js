// Generado por extraer.py desde analogia-voltaje/energia_pasa.html
// No edites este encabezado a mano; el cuerpo del sketch sí es editable.
registrarSketch('energia_pasa', {
  ancho: 750,
  alto: 320,
  pista: 'modelo energético · la bola tiene energía suficiente · click para reiniciar',
}, function(p, opciones) {
  ...
  function updateBall() {
    ...
    if (ball.x >= OIL_X + OIL_W && phase === 'inside')  phase = 'after';
    if (phase === 'inside') ball.v = Math.max(0, ball.v - DRAG);
    ball.x += ball.v;
    trail.push({ x: ball.x });
